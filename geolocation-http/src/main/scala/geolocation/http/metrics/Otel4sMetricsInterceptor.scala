...
class Otel4sMetricsRequestInterceptor[F[_]](metrics: List[Metric[F, ?]], ignoreEndpoints: Seq[AnyEndpoint]) extends RequestInterceptor[F] {
    ...
    RequestHandler.from { (request, endpoints, monad) =>
      ...
      metrics
        ...
        .flatMap { endpointMetrics =>
          requestHandler(new Otel4sMetricsEndpointInterceptor[F](endpointMetrics.reverse, ignoreEndpoints)).apply(request, endpoints)
        }
    }
}

private[metrics] class Otel4sMetricsEndpointInterceptor[F[_]](
